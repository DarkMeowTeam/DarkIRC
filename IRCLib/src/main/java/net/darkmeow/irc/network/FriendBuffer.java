...
public class FriendBuffer extends ByteBuf
{
    private final ByteBuf buf;

    public FriendBuffer(ByteBuf wrapped)
    {
        this.buf = wrapped;
    }
    ...
    public FriendBuffer writeByteArray(byte[] array)
    {
    ...
    public FriendBuffer writeVarIntArray(int[] array)
    {
        this.writeVarInt(array.length);
    ...
    public FriendBuffer writeLongArray(long[] array)
    {
        this.writeVarInt(array.length);
    ...
    public FriendBuffer writeEnumValue(Enum<?> value)
    {
    ...
    public FriendBuffer writeUniqueId(UUID uuid)
    {
    ...
    public FriendBuffer writeVarInt(int input)
    {
        while ((input & -128) != 0)
        {
    ...
    public FriendBuffer writeVarLong(long value)
    {
        while ((value & -128L) != 0L)
        {
    ...
    public FriendBuffer writeString(String string)
    {
    ...
    public FriendBuffer writeTime(Date time)
    {
    ...
    public @NotNull DataUser readUser() {
        return new DataUser(this.readString(100), this.readEnumValue(EnumUserPremium.class), this.readUserState());
    }

    public @NotNull FriendBuffer writeUser(DataUser data) {
        writeString(data.getName());
        writeEnumValue(data.getPremium());
        writeUserState(data.getState());

        return this;
    }

    public @NotNull DataUserState readUserState() {
        return new DataUserState(readPlayInGameProfile(), this.readString(100), this.readInt(), this.readBoolean());
    }

    public @NotNull FriendBuffer writeUserState(DataUserState data) {
        writePlayInGameProfile(data.getProfile());
        writeString(data.getCurrentServer());
        writeInt(data.getClientFps());
        writeBoolean(data.isFriend());

        return this;
    }

    public @NotNull DataPlayInGameProfile readPlayInGameProfile() {
        return new DataPlayInGameProfile(this.readString(100), this.readUniqueId());
    }

    public @NotNull FriendBuffer writePlayInGameProfile(DataPlayInGameProfile data) {
        this.writeString(data.getName());
        this.writeUniqueId(data.getId());
