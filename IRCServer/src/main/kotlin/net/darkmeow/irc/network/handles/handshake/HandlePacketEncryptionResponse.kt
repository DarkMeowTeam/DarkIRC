    ...
    override fun channelRead0(ctx: ChannelHandlerContext, packet: C2SPacketEncryptionResponse) {
        runCatching {
            ...
            if (connection.signatureData.isEmpty()) {
                ...
                if (!packet.verifySignature(connection.keyPair.public, connection.bossNetworkManager.base.configManager.signatureKey, connection.signatureData)) throw Exception("签名验证失败")
            }
        }
            .onFailure { e ->
